...
(** **** Exercise: 2 stars, standard, especially useful (nonzeros)
  ...
  Proof. reflexivity.  Qed.
  Example test_only_zeros: nonzeros [0] = [].
  Proof. reflexivity.  Qed.
  (** [] *)
...
(** **** Exercise: 2 stars, standard, especially useful (from_list)
    ...
    Fixpoint from_list {A : Type} (l:list A) : Vec A (length l) :=
      match l with
      | nil => vnil
      | cons e l' => vcons e (from_list l')
      end.
  
  Example to_from_list:
    to_list (from_list [3;4;2]) = [3;4;2] :=
    eq_refl.
  Example to_from_empty_list: to_list (@from_list nat []) = [] := eq_refl.
...
(** **** Exercise: 2 stars, standard, especially useful (basic_induction)
  ...
  Qed.
    ...
    Theorem mul_dist_1 : forall n k : nat,
    n * (S k) = n + n * k.
    Proof.
      intros n.
      induction n.
    ...
    *)
    ...
    Example test_nat_to_bin :  B1 (B1 Z) = nat_to_bin 3.
    Proof. reflexivity. Qed.
...
Example double_bin_zero : double_bin (B0 (B1 Z)) = B0 (B0 (B1 Z)).
Proof. reflexivity. Qed.
...
Example bin_nat_bin_test : nat_to_bin (bin_to_nat (B1 (B0 Z))) = (B1 (B0 Z)).
Abort.
...
(** A single binary number may have an infinite number of representations,
    since it can have any number of high bits, i.e. we can add any number
    of B0's applied to the Z *)
...
Example bin_normalize_test : normalize (B1 (B0 Z)) = (B1 Z).
Proof. reflexivity. Qed.

Example bin_normalize_test_bin : nat_to_bin (bin_to_nat (B1 (B0 Z))) = normalize (B1 (B0 Z)).
Proof. reflexivity. Qed.

(* Lemma dist_nat_to_bin : forall b, nat_to_bin (bin_to_nat) *)
...
Lemma double_nat : forall n, double_bin (nat_to_bin n) = nat_to_bin (double n).
Proof.
  induction n.
  - reflexivity.
  - simpl.
...
| subseq_1 : forall a tla tlb, subseq (cons a tla) (cons a tlb)
| subseq_2 : subseq nil nil.
